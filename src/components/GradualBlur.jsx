// -----------------------------------------------------------------------------
// GradualBlur.jsx
// React Bits' GradualBlur — a progressive blur ramp made of stacked
// backdrop-filter layers, each masked to its own slice of the edge so the blur
// grows smoothly instead of stepping.
//
// Used behind the feed's nav titles. The band there used to be a white-to-clear
// gradient, which only works while the page underneath is white: over the dark
// Pawmely section it read as a white bar stuck across the top. A blur ramp is
// colour-agnostic — it takes whatever is behind it and softens it — so the same
// band now works over a pale section and a black one without changing.
//
// Upstream lists `mathjs` as a dependency; the source never references it, so it
// is not installed.
// -----------------------------------------------------------------------------
